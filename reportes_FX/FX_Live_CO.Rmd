---
title: "FX Live - Análisis Técnico y de Riesgo - Colombia"
output:
  html_document:
    toc: true
    toc_depth: 3
...
current_formatted <- tibble(
  ...
  Valor = c(
    ...
    paste0("$ ", round((current_data$volatility_20 / 100 * current_data$D_Close), 3)),
...
p_top <- ggplot(data_60m, aes(x = Fecha)) +
  ...
  geom_line(aes(y = SMA200), color = "#f3ff4cff", linewidth = 0.9, alpha = 0.9) +
  labs(
...
p_bottom <- ggplot(data_24m, aes(x = Fecha)) +
  ...
  geom_ribbon(aes(ymin = Lower20, ymax = Higher20), alpha = 0.15, fill = "#fffb18ff") +
  geom_ribbon(aes(ymin = Lower50, ymax = Higher50), alpha = 0.10, fill = "#a89806ff") +
  geom_line(aes(y = SMA50), color = "#a89806ff", linewidth = 0.9) +
  geom_line(aes(y = SMA20), color = "#fffb18ff", linewidth = 0.9) +
  labs(
