---
title: "FX Live - Análisis Técnico y de Riesgo - Perú"
output:
  html_document:
    toc: true
    toc_depth: 3
...
# Tema visual
theme_fx <- function() {
  theme_grey(base_family="Century Gothic") +
    theme(
...
# Alerta para Outside50
if (current_data$Outside50) {
  alert_type <- if_else(current_data$D_Close > current_data$Higher50, "warning", "danger")
  alert_msg <- if_else(
    current_data$D_Close > current_data$Higher50, 
    "⚠️ Precio fuera de banda superior (50 períodos) - Posible sobredepreciación",
    "🚨 Precio fuera de banda inferior (50 períodos) - Posible sobreapreciación"
  )
  alerts <- append(alerts, list(list(type = alert_type, message = alert_msg)))
}
...
if (!is.na(current_data$rsi)) {
  if (current_data$rsi > 70) {
    alerts <- append(alerts, list(list(type = "warning", message = "⚠️ RSI indica condición de sobrecompra")))
  } else if (current_data$rsi < 30) {
    alerts <- append(alerts, list(list(type = "danger", message = "🚨 RSI indica condición de sobreventa")))
  }
}

# Alerta movimiento diario inusual
...
current_formatted <- tibble(
  ...
  Valor = c(
    ...
    paste0("S/ ", round((current_data$volatility_20 / 100 * current_data$D_Close), 3)),
...
p_bottom <- ggplot(data_24m, aes(x = Fecha)) +
  ...
  geom_ribbon(aes(ymin = Lower20, ymax = Higher20), alpha = 0.15, fill = "#eb7a76ff") +
  geom_ribbon(aes(ymin = Lower50, ymax = Higher50), alpha = 0.10, fill = "#6e0101ff") +
  geom_line(aes(y = SMA50), color = "#eb7a76ff", linewidth = 0.9) +
  geom_line(aes(y = SMA20), color = "#6e0101ff", linewidth = 0.9) +
  labs(
...
# Usar base R para calcular fecha límite (más confiable)
fecha_actual <- Sys.Date()
año_actual <- as.numeric(format(fecha_actual, "%Y"))
...
if(mes_actual > 6) {
  ...
  mes_limite <- mes_actual + 6  # 12 - (6 - mes_actual)
}

fecha_limite <- as.Date(paste(año_limite, sprintf("%02d", mes_limite), "01", sep = "-"))

cat("Fecha límite calculada:", as.character(fecha_limite), "\n")
...
trend_summary <- df_data %>%
  ...
  dplyr::group_by(Año_Mes) %>%
  dplyr::summarise(
    Retorno_Mensual = (dplyr::last(D_Close) / dplyr::first(D_Close) - 1) * 100,
    Fecha_Mes = dplyr::first(as.Date(paste(Año_Mes, "01", sep = "-"))),
    .groups = "drop"
  ) %>%
